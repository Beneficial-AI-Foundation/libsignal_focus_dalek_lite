...
impl ConstantTimeEq for FieldElement {
            ...
            choice_is_true(result) == (spec_fe51_to_bytes(self) == spec_fe51_to_bytes(other)),
    {
        ...
        proof {
            ...
            // 3. as_bytes postcondition: bytes32_as_nat(&bytes) == u64_5_as_nat(fe.limbs) % p()
            // 4. lemma_as_bytes_equals_spec_fe51_to_bytes: seq_from32(&bytes) == spec_fe51_to_bytes(fe)
            //    when bytes32_as_nat(&bytes) == u64_5_as_nat(fe.limbs) % p()
            // 5. Therefore: choice_is_true(result) == (spec_fe51_to_bytes(self) == spec_fe51_to_bytes(other))
            // From as_bytes() postcondition, we know:
            // - bytes32_as_nat(&self_bytes) == u64_5_as_nat(self.limbs) % p()
            // - bytes32_as_nat(&other_bytes) == u64_5_as_nat(other.limbs) % p()
...
impl FieldElement {
            ...
            choice_is_true(result) == (spec_fe51_to_bytes(self)[0] & 1 == 1),
    {
        ...
        proof {
            // From as_bytes() postcondition: bytes32_as_nat(&bytes) == u64_5_as_nat(self.limbs) % p()
